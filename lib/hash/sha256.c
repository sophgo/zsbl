...
/**
   ...
   @return CRYPT_OK if successful
*/
static int sha256_done(hash_state * md, unsigned char *out)
{
	int i;
...
int do_sha256(unsigned char *msg, unsigned char *res, unsigned long len)
{
	hash_state md;
	int ret;
	...
	ret = sha256_process(&md, msg, len);
	if (ret) {
		pr_err("sha256 process error, %d\n", ret);
		return ret;
	}

	ret = sha256_done(&md, res);
	if (ret) {
		pr_err("sha256 done error, %d\n", ret);
		return ret;
	}
